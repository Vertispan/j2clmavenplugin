...
public class TestMojo
    ...
    implements ClosureBuildConfiguration {
   ...
   * </ul>
   */
  @Parameter(defaultValue = Artifact.SCOPE_TEST,
             required = true)
  protected String classpathScope;

  @Parameter(defaultValue = "${project.artifactId}/test.js",
             required = true)
  protected String initialScriptFilename;

  @Parameter(defaultValue = "${project.build.directory}/${project.build.finalName}-test",
             required = true)
   ...
   */
  public static List<ClosureBuildConfiguration> getTestConfigs(ClosureBuildConfiguration baseConfig,
                                                               List<String> tests,
    ...
    if (tests == null || tests.isEmpty()) {
      testEntrypoints = project.getTestCompileSourceRoots()
                               .stream()
                               .flatMap(s -> getTestEntrypoints(new File(s),
                                                                includes,
                                                                excludes).stream())
                               .distinct()
                               .map(f -> f.replaceAll("\\.java$",
                                                      "")
                                          .replaceAll("/",
                                                      "."))
    ...
    return testEntrypoints.stream()
                          .map(name -> new TestConfig(name,
                                                      baseConfig))
                          .collect(Collectors.toList());
  }

  private static List<String> getTestEntrypoints(File testSourceDir,
                                                 List<String> includes,
                                                 List<String> excludes) {
  ...
  public void execute()
    ...
    List<File> extraClasspath = Arrays.asList(getFileWithMavenCoords(jreJar),
                                              ...
                                              getFileWithMavenCoords("com.vertispan.jsinterop:base:1.0.0-SNAPSHOT"),
                                              //TODO stop hardcoding this when goog releases a "base" which actually works on both platforms
                                              getFileWithMavenCoords("com.vertispan.j2cl:junit-processor:0.3-SNAPSHOT"),
                                              getFileWithMavenCoords(junitAnnotations));

    List<File> extraJsZips = Arrays.asList(getFileWithMavenCoords(jreJsZip),
                                           getFileWithMavenCoords(bootstrapJsZip),
                                           getFileWithMavenCoords(testJsZip));

    DiskCache diskCache = new DiskCache(pluginVersion,
                                        gwt3BuildCacheDir,
                                        getFileWithMavenCoords(javacBootstrapClasspathJar),
                                        extraClasspath,
                                        extraJsZips);
    diskCache.takeLock();
    ...
    try {
      CachedProject source = loadDependenciesIntoCache(project.getArtifact(),
                                                       project,
                                                       false,
                                                       projectBuilder,
                                                       request,
      ...
      children.add(source);
      CachedProject e = new CachedProject(diskCache,
                                          project.getArtifact(),
                                          project,
                                          children,
      ...
      for (ClosureBuildConfiguration config : getTestConfigs(this,
                                                             tests,
                                                             project,
                                                             includes,
                                                             excludes)) {
        e.registerAsApp(config)
         .join();
        ...
        try {
          File outputJs = new File(new File(webappDirectory),
                                   config.getInitialScriptFilename());
          Path junitStartupFile = Paths.get(outputJs.getParent(),
                                            outputJs.getName()
                                                    .substring(0,
                                                               outputJs.getName()
                                                                       .length() - 2) + "html");
          ...
          fileContents = fileContents.replace("<TEST_SCRIPT>",
                                              outputJs.getName());

          Files.write(junitStartupFile,
                      fileContents.getBytes("UTF-8"),
                      StandardOpenOption.CREATE,
                      StandardOpenOption.TRUNCATE_EXISTING);
          startupHtmlFile = junitStartupFile.toAbsolutePath()
                                            .toString();
      ...
      throw new MojoExecutionException("Failed to build project structure",
                                       e);
    }
    ...
    if (failedTests.isEmpty()) {
      ...
      failedTests.forEach((name, startupHtmlFile) -> getLog().error(String.format("Test %s failed, please try manually %s",
                                                                                  name,
                                                                                  startupHtmlFile)));
  ...
  private static class TestConfig
    ...
    private TestConfig(String test,
                       ClosureBuildConfiguration wrapped) {
      this.test = test;
    ...
    @Override
    public String getInitialScriptFilename() {
      return wrapped.getInitialScriptFilename()
                    .substring(0,
                               ".js".length()) + "-" + test + ".js";
    }
