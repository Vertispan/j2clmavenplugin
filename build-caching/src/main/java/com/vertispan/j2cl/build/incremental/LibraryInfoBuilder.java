...
public final class LibraryInfoBuilder {
 ...
 /* public static final int NULL_TYPE = 0;
    ...
    member.accept(
        new AbstractVisitor() {
          ...
          public void exitFieldAccess(FieldAccess node) {
            ...
            addInvokedMethod(target, node.getQualifier() != null ? (DeclaredTypeDescriptor) node.getQualifier().getTypeDescriptor() : null);
          }
          ...
          private void addInvokedMethod(MemberDescriptor target, DeclaredTypeDescriptor nodeEnclosingType) {
            invokedMethods.add(createMethodInvocation(target, nodeEnclosingType));
  ...
  private MethodInvocation createMethodInvocation(MemberDescriptor memberDescriptor, DeclaredTypeDescriptor nodeEnclosingType) {
    return MethodInvocation.newBuilder()
        .setMethod(getMemberId(memberDescriptor))
        .setTargetEnclosingType(getTypeId(memberDescriptor.getEnclosingTypeDescriptor()))
        .setNodeEnclosingType((nodeEnclosingType != null) ? getTypeId(nodeEnclosingType) : -1)
        .build();
  }
